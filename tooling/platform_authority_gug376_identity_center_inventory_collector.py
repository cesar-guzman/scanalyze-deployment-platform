...
LIVE_DISCOVERY_SUPPLEMENT_SHA256 = "46194be285ed0a07f72e8a5d2a90e88e04bacfbc1a32a42057b209dfee32133c"
...
LIVE_POLICY_TARGET_FIELDS = POLICY_TARGET_FIELDS | {"identity_center_kms_mode"}
...
LIVE_BINDING_FIELDS = BINDING_FIELDS | {
    "identity_center_instance_arn",
    "identity_center_kms_binding_digest",
}
...
def _validate_live_private(private: Mapping[str, Any]) -> None:
    ...
    if (
        ...
        or _INSTANCE_ARN.fullmatch(
            str(private.get("identity_center_instance_arn"))
        )
        is None
        or private.get("identity_center_kms_binding_digest")
        != canonical_digest(binding)
    ):
        _fail("IDENTITY_CENTER_PRIVATE_TARGET_INVALID")


def _valid_kms_binding(
    value: Mapping[str, Any], *, account: str, live: bool
) -> bool:
...
def _valid_policy_targets(value: object, *, account: str, live: bool) -> bool:
    expected_fields = LIVE_POLICY_TARGET_FIELDS if live else POLICY_TARGET_FIELDS
    if not isinstance(value, Mapping) or set(value) != expected_fields:
        return False
    if value.get("management_account_id") != account or not _valid_kms_binding(
        value, account=account, live=live
    ):
        return False
    return all(
        key in {"management_account_id", "identity_center_kms_mode", "identity_center_kms_key_arn"}
        or _ARN.fullmatch(str(item)) is not None
...
def _validate_plan(plan: Mapping[str, Any]) -> Mapping[str, Any]:
    ...
    live = isinstance(private, Mapping) and set(private) == LIVE_PRIVATE_FIELDS
    ...
    if (
        ...
        or set(private) not in (PRIVATE_FIELDS, LIVE_PRIVATE_FIELDS)
        or any(
            key not in {"identity_center_kms_key_arn"}
            and (not isinstance(item, str) or not item or "*" in item)
            for key, item in private.items()
        )
        or any(_DIGEST.fullmatch(str(plan[key])) is None for key in digests)
        or not isinstance(plan["expected_account_id"], str)
        or re.fullmatch(r"[0-9]{12}", plan["expected_account_id"]) is None
    ):
    ...
    if (
        any(_ARN.fullmatch(str(private[key])) is None for key in ("approved_single_operator_user_arn", "authority_account_arn", "identity_store_arn"))
        or not _valid_kms_binding(private, account=account, live=live)
        or store not in private["identity_store_arn"]
        or store not in private["approved_single_operator_user_arn"]
        or not private["approved_single_operator_user_arn"].endswith("/" + user)
        or re.fullmatch(r"arn:aws:sso:::account/[0-9]{12}", private["authority_account_arn"]) is None
        or re.fullmatch(rf"arn:aws:sts::{account}:assumed-role/[A-Za-z0-9+=,.@_/-]+/[A-Za-z0-9+=,.@_-]+", str(plan["expected_principal_arn"])) is None
    ):
        _fail("IDENTITY_CENTER_PRIVATE_TARGET_INVALID")
    return private
def _render(
    ...
    *,
    ...
    private = _validate_plan(plan)
    if live != (set(private) == LIVE_PRIVATE_FIELDS) or live_discovery and not live:
        _fail("IDENTITY_CENTER_LIVE_POLICY_STAGE_INVALID")
    ...
    if live:
        for statement in policy["Statement"]:
            if statement.get("Sid") == "DiscoverExactIdentityCenterApplication":
                condition = statement.get("Condition")
                equals = condition.get("StringEquals") if isinstance(condition, dict) else None
                if not isinstance(equals, Mapping) or equals != {
                    "aws:RequestedRegion": REGION,
                    "sso:ApplicationAccount": "${management_account_id}",
                    "sso:PrimaryRegion": REGION,
                }:
                    _fail("IDENTITY_CENTER_LIVE_POLICY_SOURCE_DRIFT")
            ...
            if statement.get("Action") == "kms:Decrypt":
                ...
                equals.update(
                    {
                        "aws:PrincipalAccount": plan["expected_account_id"],
                        "aws:RequestedRegion": REGION,
                        "kms:CallerAccount": plan["expected_account_id"],
                    }
                )
    if live_discovery:
        ...
        permission_set_lists[0]["Resource"] = private[
            "identity_center_instance_arn"
        ]
        ...
        if not isinstance(statements, list) or len(statements) != 3 or any(not isinstance(item, Mapping) for item in statements) or {item.get("Sid") for item in statements} != {"ReadBoundIdentityCenterInstance", "DiscoverPermissionSetNames", "DecryptDiscoveryMetadataThroughIdentityCenter"}: _fail("IDENTITY_CENTER_LIVE_POLICY_SOURCE_DRIFT")
        policy["Statement"].extend(statements)
    kms_mode = (
        private["identity_center_kms_mode"]
        if live
    ...
    if kms_mode == "AWS_OWNED_KMS_KEY":
        policy["Statement"] = [
            ...
            and not (
                isinstance(item.get("Action"), list)
                and "kms:Decrypt" in item["Action"]
            )
        ]
        for item in policy["Statement"]:
            not_actions = item.get("NotAction")
            if isinstance(not_actions, list):
                item["NotAction"] = [
                    action for action in not_actions if action != "kms:Decrypt"
                ]
    ...
    if live:
        values["identity_center_instance_arn"] = private[
            "identity_center_instance_arn"
        ]
        values["identity_center_instance_id"] = private[
            "identity_center_instance_arn"
        ].rsplit("/", 1)[-1]
    if private["identity_center_kms_key_arn"] is not None:
        values["identity_center_kms_key_arn"] = private["identity_center_kms_key_arn"]
    if targets is not None:
        if not _valid_policy_targets(
            targets,
            account=str(plan["expected_account_id"]),
            live=live,
        ):
            _fail("IDENTITY_CENTER_POLICY_TARGET_SET_INVALID")
        if targets["identity_center_kms_key_arn"] != private[
            "identity_center_kms_key_arn"
        ] or live and targets["identity_center_kms_mode"] != kms_mode:
            _fail("IDENTITY_CENTER_POLICY_TARGET_SET_INVALID")
        values.update(
            {
                key: value
                for key, value in targets.items()
                if value is not None and key != "identity_center_kms_mode"
            }
        )
...
def render_policy(plan: Mapping[str, Any], targets: Mapping[str, Any] | None = None) -> tuple[dict[str, Any], str]:
    policy, digest = _render(plan, targets, live=False); expected = plan["expected_discovery_policy_digest" if targets is None else "expected_exact_policy_digest"]
...
def render_live_policy(plan: Mapping[str, Any], targets: Mapping[str, Any] | None = None) -> tuple[dict[str, Any], str]:
    ...
    policy, digest = _render(plan, targets, live=True, live_discovery=targets is None); expected = plan["expected_discovery_policy_digest" if targets is None else "expected_exact_policy_digest"]
    if digest != expected or targets is not None and canonical_digest(targets) != plan["expected_target_digest"]: _fail("IDENTITY_CENTER_POLICY_DIGEST_NOT_VERIFIED")
    return policy, digest
def render_live_policy_candidate(plan: Mapping[str, Any], targets: Mapping[str, Any] | None = None) -> tuple[dict[str, Any], str]:
    _validate_live_private(_validate_plan(plan))
    return _render(plan, targets, live=True, live_discovery=targets is None)
...
def _discover(
    ...
    *,
    ...
    instances = (
        _pages(reader.list_instances) if instances is None else list(instances)
    )
    instance = str(instances[0].get("instance_arn", "")) if len(instances) == 1 else ""
    expected_instance = private.get("identity_center_instance_arn")
...
def _bind(plan: Mapping[str, Any], discovery: Mapping[str, list[dict[str, Any]]], *, live: bool = False) -> tuple[str, dict[str, Any]]:
    ...
    if not applications and not permission_sets:
        ...
        if (
            live
            and len(instances) == 1
            and instances[0].get("instance_arn")
            == private["identity_center_instance_arn"]
...
def _valid_live_instance_binding(
    instance: object, *, binding: Mapping[str, Any]
) -> bool:
    ...
    return (
        ...
        == binding.get("identity_center_instance_arn")
        and instance.get("owner_account_id") == account
...
def _read_live_instance(
    reader: DiscoveryReader, plan: Mapping[str, Any]
) -> dict[str, Any]:
    private = plan["private_targets"]
    instance = _one(
        reader.describe_instance(private["identity_center_instance_arn"]),
        {
            "instance_arn",
            "identity_store_id",
            "owner_account_id",
    ...
    binding, _ = plan_binding(plan)
    if not _valid_live_instance_binding(instance, binding=binding):
        _fail("IDENTITY_CENTER_KMS_BINDING_MISMATCH")
    return instance
...
def _valid_live_exact_shape(
    ...
    if any(
        ...
        return False
    if (
        not _valid_policy_targets(
            targets,
            account=str(targets.get("management_account_id", "")),
            live=True,
        )
...
def _valid_exact(
    ...
    return True
def _exact(reader: ExactReader, plan: Mapping[str, Any], targets: Mapping[str, Any], discovery: Mapping[str, Any], *, live: bool = False) -> tuple[dict[str, Any], dict[str, int | None]]:
    ...
    if live:
        binding, _ = plan_binding(plan)
        if not _valid_live_instance_binding(
            facts["instance"], binding=binding
        ):
            _fail("IDENTITY_CENTER_KMS_BINDING_MISMATCH")
    ...
    return facts, counts
def plan_binding(plan: Mapping[str, Any]) -> tuple[dict[str, Any], str]:
    private = _validate_plan(plan)
    value = {
        key: plan[key]
        for key in PLAN_FIELDS - {"private_targets", "not_before", "not_after"}
    }
    value.update(
        {
            ...
            "not_before": _stamp(_time(plan["not_before"])),
            "not_after": _stamp(_time(plan["not_after"])),
            "region": REGION,
        }
    )
    if set(private) == LIVE_PRIVATE_FIELDS:
        value.update(
            {
...
def _capture(plan: Mapping[str, Any], factory: DirectSessionFactory, *, private_root: Path, artifact_name: str, now: datetime, live: bool, validation_clock: Callable[[], datetime] | None, exact_plan_materializer: Callable[[Mapping[str, Any], Mapping[str, Any], object], Mapping[str, Any]] | None = None) -> dict[str, Any]:
    ...
    empty_counts = {key: None for key in COUNTS}
    discovery_instance: dict[str, Any] | None = None
    transition_attestation: object | None = None
    if live:
        instances, failure = _provider(
            lambda: _pages(discovery_reader.list_instances)
        )
        if failure:
            return _save(
                private_root,
        ...
        if (
            len(instances) == 1
            and instances[0].get("instance_arn")
            == plan["private_targets"]["identity_center_instance_arn"]
        ):
            discovery_instance, failure = _provider(
                lambda: _read_live_instance(discovery_reader, plan)
            )
            if failure:
                return _save(
                    private_root,
        ...
        discovery, failure = _provider(
            lambda: _discover(discovery_reader, plan)
        )
    if failure:
        return _save(private_root, artifact_name, plan, failure, policies, {}, {"classification": failure}, identities, empty_counts, live=live)
    ...
    if discovery_instance is not None and exact_plan_materializer is not None:
        try:
            transition_attestation = discovery_reader.attest_transition(
                canonical_digest(
                    {"discovery": discovery, "instance": discovery_instance}
                )
            )
        except CollectorError:
            raise
        except Exception as exc:
            raise CollectorError("DISCOVERY_EXACT_TRANSITION_INVALID") from exc
    ...
    if classification != "EXACT_PRESENT_NO_TOUCH" and exact_plan_materializer is not None and targets:
        ...
        try:
            if transition_attestation is None:
                _fail("DISCOVERY_EXACT_TRANSITION_INVALID")
    ...
    if classification != "EXACT_PRESENT_NO_TOUCH":
        discovery_facts: dict[str, Any] = {"discovery": discovery}
        if discovery_instance is not None:
            discovery_facts["instance"] = discovery_instance
        return _save(private_root, artifact_name, plan, classification, policies, targets, discovery_facts, identities, counts, live=live)
...
def _outcome(value: Mapping[str, Any], binding: Mapping[str, Any]) -> tuple[str, dict[str, int | None]]:
    ...
    live = value.get("record_type") == "scanalyze.platform_authority.gug376_identity_center_inventory_private.v2"
    ...
    if keys == {"discovery"} or live and keys == {"discovery", "instance"}:
        ...
        instance_evidence_valid = (
            not live
            or "instance" not in facts
            or _valid_live_instance_binding(facts["instance"], binding=binding)
        )
        if live:
            absence = (
                keys == {"discovery", "instance"}
                and _valid_live_instance_binding(
                    facts["instance"], binding=binding
                )
                and
                bounded
        ...
        if value["classification"] == "DRIFT_BLOCKED_NO_REPAIR" and instance_evidence_valid and set(value["policies"]) == {"discovery"} and (not targets and not complete and not absence or value["target_digest"] != binding["expected_target_digest"] and _valid_policy_targets(targets, account=str(binding["expected_account_id"]), live=live) and _bound_discovery(discovery, targets)): return "DRIFT_BLOCKED_NO_REPAIR", counts
    ...
    if keys != full or set(value["policies"]) != {"discovery", "exact"} or not _valid_policy_targets(targets, account=str(binding["expected_account_id"]), live=live) or live and not _valid_live_instance_binding(facts.get("instance"), binding=binding) or value["target_digest"] != binding["expected_target_digest"] or any(not isinstance(item, Mapping) or set(item) != set(NAMES) for item in surfaces) or any(not isinstance(facts.get(key), Mapping) for key in ("instance", "application", "operator")): _fail("PRIVATE_SNAPSHOT_INVALID")
    if not _valid_exact(facts, targets, live=live): _fail("PRIVATE_SNAPSHOT_INVALID")
...
def _snapshot(value: Mapping[str, Any], *, live: bool = False) -> dict[str, Any]:
    ...
    expected_binding_fields = LIVE_BINDING_FIELDS if live else BINDING_FIELDS
    ...
    if live:
        digest_keys.add("identity_center_kms_binding_digest")
    if set(binding) != expected_binding_fields or any(_DIGEST.fullmatch(str(binding.get(key))) is None for key in digest_keys) or live and _INSTANCE_ARN.fullmatch(str(binding.get("identity_center_instance_arn"))) is None or binding.get("region") != REGION or not isinstance(binding.get("expected_account_id"), str) or re.fullmatch(r"[0-9]{12}", binding["expected_account_id"]) is None or re.fullmatch(rf"arn:aws:sts::{binding.get('expected_account_id')}:assumed-role/[A-Za-z0-9+=,.@_/-]+/[A-Za-z0-9+=,.@_-]+", str(binding.get("expected_principal_arn"))) is None or value["plan_binding_digest"] != canonical_digest(binding): _fail("PRIVATE_SNAPSHOT_INVALID")
