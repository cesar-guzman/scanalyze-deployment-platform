...
variable "customer_id" {
  type        = string
  default     = null
  description = "Independent customer authority; required by baseline storage ownership."
}
...
locals {
  ...
  artifact_kms_key_arn = local.storage_baseline ? try(data.aws_kms_key.application_storage[0].arn, null) : aws_kms_key.artifacts[0].arn
  artifact_kms_key_id  = local.storage_baseline ? try(data.aws_kms_key.application_storage[0].id, null) : aws_kms_key.artifacts[0].id
}

data "aws_kms_key" "application_storage" {
  count  = local.storage_baseline && local.storage_receipt_admitted ? 1 : 0
  key_id = var.application_storage_foundation.keys.cicd_artifacts.key_arn
}
data "aws_kms_alias" "application_storage" {
  count = local.storage_baseline && local.storage_receipt_admitted ? 1 : 0
  name  = local.storage_aliases.cicd_artifacts
}
...
resource "terraform_data" "application_storage_gate" {
  ...
  triggers_replace = [var.kms_ownership_mode, local.artifact_kms_key_arn]
  lifecycle {
    ...
    precondition {
      condition = !local.storage_baseline || try(
        data.aws_kms_key.application_storage[0].arn == var.application_storage_foundation.keys.cicd_artifacts.key_arn &&
        data.aws_kms_key.application_storage[0].id == var.application_storage_foundation.keys.cicd_artifacts.key_id &&
        ...
        data.aws_kms_alias.application_storage[0].target_key_id == var.application_storage_foundation.keys.cicd_artifacts.key_id &&
        data.aws_kms_alias.application_storage[0].target_key_arn == var.application_storage_foundation.keys.cicd_artifacts.key_arn,
        false
      )
...
moved {
  ...
  to   = aws_kms_key.artifacts[0]
}
moved {
  from = aws_kms_alias.artifacts
  to   = aws_kms_alias.artifacts[0]
}
