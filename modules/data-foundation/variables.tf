...
variable "platform_contract" {
  ...
  description = "Contract payload from platform layer (platform/v1)"
}

variable "enable_replicated_data" {
  type        = bool
  default     = false
  description = "Enable replicated-data sub-module (future HA/DR). Must be false in M1."
}
