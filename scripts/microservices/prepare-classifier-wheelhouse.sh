...
SERVICE_DIR="${REPO_ROOT}/backend/workers/scanalyze-classifier-worker"
LOCK_FILE="${SERVICE_DIR}/requirements.lock"
WHEELHOUSE_DIR="${SERVICE_DIR}/.wheelhouse"
MARKER_FILE="${WHEELHOUSE_DIR}/.gug378-wheelhouse"
CLASSIFIER_PYTHON_BIN="${CLASSIFIER_PYTHON_BIN:-python3.11}"
...
command -v "$CLASSIFIER_PYTHON_BIN" >/dev/null 2>&1 ||
  die "Python 3.11.14 is required"
[[ "$($CLASSIFIER_PYTHON_BIN --version 2>&1)" == "Python 3.11.14" ]] ||
...
PIP_DISABLE_PIP_VERSION_CHECK=1 PIP_NO_INPUT=1 \
  "$CLASSIFIER_PYTHON_BIN" -m pip download \
    --dest "$temporary_dir" \
    --require-hashes \
    --only-binary=:all: \
...
[[ "$wheel_count" == "13" ]] || die "expected 13 locked wheels, found ${wheel_count}"
if find "$temporary_dir" -maxdepth 1 -type l | grep -q .; then
  die "wheelhouse must not contain symbolic links"
fi

printf '%s\n' "$expected_marker" > "${temporary_dir}/.gug378-wheelhouse"
...
temporary_dir=""
printf 'Prepared %s verified classifier wheels for linux/amd64 CPython 3.11.14.\n' "$wheel_count"
