#!/usr/bin/env python3
"""Build the deterministic GUG-376 repair Lambda ZIP offline."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build the source-closed deterministic GUG-376 repair Lambda "
            "package; this command performs no AWS operation"
        )
    )
    ...
    parser.add_argument(
        ...
        type=Path,
        help=(
            "A new directory outside the repository; existing paths are "
            "rejected"
        ),
    )
...
def main() -> int:
    ...
    try:
        ...
        print(f"GUG376_PACKAGE_BLOCKED:{exc}", file=sys.stderr)
        return 2
    print(
        canonical_json(
            {
