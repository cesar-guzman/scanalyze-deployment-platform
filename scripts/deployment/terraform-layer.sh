...
if [[ "$ACTION" == "apply" ]]; then
  die "Local Terraform apply is disabled by ADR-017. This change provides dry-run orchestration only."
fi
...
case "$LAYER" in
  cicd|services)
    ...
    ;;
  edge)
    export TF_VAR_upstream_contract_id="${TF_VAR_upstream_contract_id:-edge-identity/v2}"
    export TF_VAR_upstream_schema_version="${TF_VAR_upstream_schema_version:-2}"
    ;;
  *)
...
export TF_VAR_internal_certificate_arn="${TF_VAR_internal_certificate_arn:-arn:aws:acm:${REGION}:${ACCOUNT_ID}:certificate/00000000-0000-0000-0000-000000000000}"
export TF_VAR_domain_name="${TF_VAR_domain_name:-example.com}"
export TF_VAR_alb_listener_arn="${TF_VAR_alb_listener_arn:-arn:aws:elasticloadbalancing:${REGION}:${ACCOUNT_ID}:listener/app/mock/0000000000000000/0000000000000000}"
export TF_VAR_alb_security_group_id="${TF_VAR_alb_security_group_id:-sg-00000000000000000}"
export TF_VAR_api_access_log_group_arn="${TF_VAR_api_access_log_group_arn:-arn:aws:logs:${REGION}:${ACCOUNT_ID}:log-group:/aws/apigateway/mock:*}"
...
export TF_VAR_ecs_cluster_arn="${TF_VAR_ecs_cluster_arn:-arn:aws:ecs:${REGION}:${ACCOUNT_ID}:cluster/mock}"
export TF_VAR_ecs_task_execution_role_arn="${TF_VAR_ecs_task_execution_role_arn:-arn:aws:iam::${ACCOUNT_ID}:role/mock-exec-role}"
export TF_VAR_workload_role_arns="${TF_VAR_workload_role_arns:-{ \"auth\" = \"arn:aws:iam::${ACCOUNT_ID}:role/mock-workload-role\" }}"
...
case "$ACTION" in
  ...
  *)
    die "Unknown action: ${ACTION}. Only local plan is supported."
    ;;
esac
