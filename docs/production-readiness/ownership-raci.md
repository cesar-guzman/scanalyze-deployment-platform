...
| `data-foundation` | Registry tuple and `platform/v1` | `roots/data-foundation` Apply execution produces `data-foundation/v1` | Regional data-foundation key owned by data-foundation root | PE | Local root/contract evidence; live Blocked |
| `cicd` | Registry tuple, `data-foundation/v1`, reviewed source/repository policy | `roots/cicd` Apply execution produces `cicd/v1` | Regional cicd key owned by cicd root | PE | Local root/contract evidence; live Blocked |
...
| `services` | Registry tuple, `platform/v1`, `data-foundation/v1`, `cicd/v1`, and `release-manifest/v1` | `roots/services` Apply execution produces `services/v1` | Regional services key owned by services root | PE | Local ownership/contract evidence; live Blocked |
