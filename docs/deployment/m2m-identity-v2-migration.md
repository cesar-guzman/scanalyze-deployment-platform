...
> **Owner issues:** GUG-102, GUG-92, GUG-93, GUG-114, GUG-117
> **Production:** NO-GO
...
| API Gateway/Cognito without the M2M audience, claims, or canonical GUG-92 policy versions/scopes | Declarative identity contract does not match runtime | Add and validate through GUG-93; do not compensate in application code | GUG-93 / GUG-92 |
...
4. Approve the exact, non-empty, pairwise-disjoint action scope sets from
   ADR-023: `scanalyze.api.v1/read`, `scanalyze.api.v1/write`, and
   `scanalyze.api.v1/admin` for `read`, `write`, and `admin` respectively.
...
6. Generate an identity v2 contract and validate its schema and semantic
   bindings offline.
7. Bind the supported authorization schema, action-scope catalog, policy digest,
   and workload grant versions. Unknown, stale, conflicting, or future versions
   deny rather than falling back to an older catalog.
8. Render task-definition input v2 and verify exactly one canonical customer
   and deployment environment entry, with no extension override.
9. Keep M2M disabled until Cognito, API Gateway, services handoff, and runtime
   configuration are part of one reviewed non-production change.
10. Run positive and negative tests for the exact deployment plus a second
   isolated deployment. Any cross-deployment success is a P0 failure.
11. Verify that read-only bindings cannot write, download, export, or retrieve
    full PII, and that token-only extra scopes do not elevate the binding.
12. Record sanitized evidence with commit, release, environment, test result,
   reviewer, and rollback reference.
13. Enable production only through the later production gate; this runbook does
...
- Unknown, stale, conflicting, or unsupported policy/grant version: deny and
  keep the workload disabled; never reinterpret it as the current version.
