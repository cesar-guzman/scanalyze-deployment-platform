...
| Wildcard action pattern (e.g. `lambda:*`, `*`, `?`, `[`) grants broad authority | Centralized classifier detects metacharacters **before** fnmatch expansion and emits specifically classified PROHIBITED edges when a canonical authority action is covered | `WILDCARD_ACTION` / `FOREIGN_AUTHORITY_PRESENT` |
| Wildcard action pattern understates or overstates mutation authority surface | One shared coverage result independently matches invocation, Lambda mutation, IAM mutation and CloudFormation mutation catalogs | Covered classes emit `PROHIBITED`; mutation-first precedence |
| Relevant wildcard such as `lambda:Get*` is mislabeled as invocation | A relevant wildcard that matches no canonical authority catalog is unclassified and fails closed | `POLICY_SEMANTICS_UNSUPPORTED` |
...
| Wildcard action scoped to an unrelated Lambda function ARN blocks the target inventory | Resource/NotResource applicability is validated before emitting wildcard invocation edges; `_resource_applies()` and `_is_target_function_pattern()` gate emission | No invocation edge for unreachable targets; `unsupported` for malformed/variable resources |
| Future qualifier wildcard receives an exact-safe receipt before the alias exists | A target `Resource` glob such as `<function-arn>:future-*` is latent authority; `NotResource` remains candidate-based | `FOREIGN_INVOCATION_AUTHORITY`; no exact receipt |
...
| Mixed exact-plus-wildcard statement loses mutation precedence | Atomic coverage includes exact siblings while suppressing exact allowlist edges | No partial success; `AUTHORITY_MUTATION_PRESENT` precedes invocation |
| GUG-221 consumer maps every wildcard to unsupported or invocation before resource evaluation | GUG-221 consumes the shared classifier and `_resource_applies()` result for plan/repair/reconcile targets | Specific mutation/invocation code only after `Resource`/`NotResource` applicability |
