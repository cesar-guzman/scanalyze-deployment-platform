...
| partial bootstrap emits success | closed readback requires exactly three buckets, three KMS keys, eight roles, and six exact controls | missing, duplicated, unexpected, or false control fails; no output survives |
...
| a one-hour IAM role maximum is reported as a 15-minute session cap | the identity contract requires `DurationSeconds=900`, while the child honestly uses IAM's minimum supported `MaxSessionDuration` of 3,600 seconds | broker/caller enforcement and negative live evidence remain downstream; the template alone does not prove the 15-minute duration |
...
| raw saved plan becomes immutable long-lived evidence | `plan-execution/` is isolated in the non-Object-Lock state bucket with short lifecycle; evidence bucket is COMPLIANCE WORM for sanitized output only | Apply cannot put/delete saved plans; plan paths are bound to session change/layer and never use the evidence bucket |
