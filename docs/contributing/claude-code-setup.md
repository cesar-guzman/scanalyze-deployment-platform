...
repository baseline, and use it day to day on this repo. Read alongside the
[Contributor Guide](contributor-guide.md) and the
...
- `model: opusplan` with pinned IDs `claude-opus-4-8` (plan) and
  `claude-sonnet-5` (execute).
- `permissions.defaultMode: plan`.
- `deny` / `ask` rules for secrets, destructive Git, remote publish, Terraform
  and AWS mutation (full list in the
  [AI-Assisted Development Standard](ai-assisted-development.md)).
...
You can also confirm the static contract without a live session:
...
1. **Start in the issue's worktree.** One Linear issue → one branch → one
   worktree (see the [Contributor Guide](contributor-guide.md)).
...
5. **Let it run gated.** Denied actions are blocked; `ask` actions pause for your
   confirmation. Never approve an AWS or `terraform apply/destroy` action — the
   baseline blocks these, and so should you.
6. **Run gates yourself.** `make contributor-check`, `make preflight-core`, and
   any narrower gate relevant to the change.
...
| `make contributor-check` fails on routing | Baseline drift | Restore `.claude/settings.json` pinned values |
| Agent edits a `.tfvars`/state file | Should be denied by baseline | Verify `.claude/settings.json` deny list is intact |
