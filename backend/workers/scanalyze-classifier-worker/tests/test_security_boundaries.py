...
def test_config_rejects_invalid_deployment_id_before_aws_client(monkeypatch):
    monkeypatch.setenv("SCANALYZE_ENV", "test")
    monkeypatch.setenv("SCANALYZE_TENANT", "platform")
    monkeypatch.setenv(
        "SCANALYZE_DEPLOYMENT_CUSTOMER_ID", "cust_01ARZ3NDEKTSV4RRFFQ69G5FAW"
    )
    monkeypatch.setenv("SCANALYZE_DEPLOYMENT_ID", "synthetic-invalid")
    with patch("classifier_worker.config.boto3.client") as boto_client:
        with pytest.raises(
            RuntimeError, match="SCANALYZE_DEPLOYMENT_ID is invalid"
        ):
            ConfigCache()
    boto_client.assert_not_called()
