...
def setup_logging():
    ...
    if not tenant:
        tenants = [
            item.strip()
            for item in os.environ.get('SCANALYZE_TENANTS', '').split(',')
            if item.strip()
        ]
        if not tenants:
            raise RuntimeError("SCANALYZE_TENANT or SCANALYZE_TENANTS is required")
        tenant = "multi"
    handler.setFormatter(JSONFormatter(tenant=tenant))
...
def log_event(event_name: str, stage: str, document_id: str = None, **kwargs):
    struct_logger = logging.getLogger('postprocess_worker.structured')
    ...
    extra = {"event": event_name, "stage": stage, **safe_kwargs}
    if document_id:
        extra["documentId"] = document_id
        
    struct_logger.info(event_name, extra=extra)
