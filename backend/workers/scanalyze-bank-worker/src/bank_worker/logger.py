...
class JSONFormatter(logging.Formatter):
    ...
    def format(self, record):
        ...
        # Filter PII: never log OCR text, transaction details, or account holder names
        for k, v in record.__dict__.items():
            if k not in ["args", "asctime", "created", "exc_info", "exc_text", "filename", "funcName", "levelname", "levelno", "lineno", "module", "msecs", "message", "msg", "name", "pathname", "process", "processName", "relativeCreated", "stack_info", "thread", "threadName", "event"]:
                kl = k.lower()
                if "text" not in kl and "ocr" not in kl and "transaction" not in kl and "holder" not in kl and "account" not in kl:
                    log_record[k] = v
...
def setup_logging():
    ...
    handler.setFormatter(JSONFormatter(tenant=tenant, stage='bank_extract'))
...
def log_event(event_name: str, **kwargs):
    logger = logging.getLogger('bank_worker.structured')
    
    # Filter PII from event kwargs
    safe_kwargs = {k: v for k, v in kwargs.items() if "text" not in k.lower() and "ocr" not in k.lower() and "transaction" not in k.lower() and "holder" not in k.lower()}
