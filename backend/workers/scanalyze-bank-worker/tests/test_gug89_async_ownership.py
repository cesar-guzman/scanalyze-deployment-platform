...
def _authorized_record(status="BANK_EXTRACTED", *, include_structured=True):
...
@pytest.mark.parametrize(
    ("field", "value"),
    [
        ("schemaVersion", "scanalyze.bank-extract.v1"),
        ("customer_id", None),
        ("deployment_id", None),
...
def test_extract_contract_rejects_legacy_or_unbound_messages(field, value):
    ...
    with pytest.raises(ValidationError):
        BankExtractMessage(**payload)


def test_invalid_message_is_not_acknowledged_or_processed():
    with (
        patch("bank_worker.processors.extract.require_authorized_document") as authorize,
        patch("bank_worker.processors.extract.is_already_extracted") as idempotency,
        ...
        assert process_bank_extract_message(json.dumps({"documentId": "legacy"}), "r", "q") is False
...
def test_runtime_owner_mismatch_has_zero_protected_effects(field, value):
    ...
    with (
        patch("bank_worker.processors.extract.config.get") as config_get,
        patch("bank_worker.processors.extract.require_authorized_document") as authorize,
        patch("bank_worker.processors.extract.is_already_extracted") as idempotency,
        patch("bank_worker.processors.extract.get_ocr_text") as s3_read,
        patch("bank_worker.processors.extract.invoke_bedrock_bank_statement") as bedrock,
        ...
        with pytest.raises(ValueError, match="runtime deployment"):
            process_bank_extract_message(json.dumps(payload), "r", "q")
...
def test_metadata_extra_field_fails_closed_before_protected_effects():
    payload = _message(_metadata={"correlationId": "corr-123", "customer_id": CUSTOMER_ID})
    with (
        patch("bank_worker.processors.extract.config.require_owner") as runtime_owner,
        patch("bank_worker.processors.extract.require_authorized_document") as authorize,
        patch("bank_worker.processors.extract.is_already_extracted") as idempotency,
        patch("bank_worker.processors.extract.get_ocr_text") as s3_read,
        patch("bank_worker.processors.extract.invoke_bedrock_bank_statement") as bedrock,
        ...
        assert process_bank_extract_message(json.dumps(payload), "r", "q") is False
...
def test_authorized_idempotent_message_propagates_owner_and_safe_metadata():
    ...
    with (
        ...
        assert process_bank_extract_message(json.dumps(payload), "r", "q") is True
    ...
    assert outbound["processing_domain"] == "bank"
...
def test_dynamo_authorization_requires_exact_owner_domain_and_locators():
    ...
    with patch("bank_worker.dynamo.dynamodb_resource.Table", return_value=table):
        item = require_authorized_document(
            "documents", "doc-123", "bank", CUSTOMER_ID, DEPLOYMENT_ID,
            "raw-trusted", "raw/doc-123.pdf", "ocr-trusted", "ocr/doc-123.json",
        )
        with pytest.raises(ValueError, match="Document is unavailable"):
            require_authorized_document(
                "documents", "doc-123", "bank",
                "cust_01ARZ3NDEKTSV4RRFFQ69G5FAX", DEPLOYMENT_ID,
                "raw-trusted", "raw/doc-123.pdf", "ocr-trusted", "ocr/doc-123.json",
            )
