...
from personal_worker.dynamo import (
...
def _writing_record():
    record = _authorized_record("PERSONAL_EXTRACTING", include_structured=False)
    record["stages"] = {
        "personal_extract": {
            "status": "WRITING",
            "artifact": build_artifact_binding(
...
def test_terminal_duplicate_is_noop(status):
    with (
        ...
        patch("personal_worker.processors.extract.update_document_status") as update,
        patch(
            "personal_worker.processors.extract.require_structured_artifact_proof",
...
def test_structured_write_is_create_only_and_never_overwrites():
    with patch("personal_worker.s3.s3_client.put_object", return_value={}) as put_object:
        created, digest = personal_s3.save_structured_artifact(
            "structured-trusted",
            _structured_key(),
            {"documentId": "doc-123"},
            **_artifact_kwargs(),
...
def test_oversized_structured_artifact_is_rejected_before_s3_write():
    oversized = {"payload": "x" * personal_s3.MAX_STRUCTURED_ARTIFACT_BYTES}
    with patch("personal_worker.s3.s3_client.put_object") as put_object:
        with pytest.raises(ValueError, match="unavailable"):
            personal_s3.save_structured_artifact(
                "structured-trusted",
                _structured_key(),
                oversized,
                **_artifact_kwargs(),
...
def test_structured_precondition_failure_reports_collision_without_overwrite():
    ...
    with patch("personal_worker.s3.s3_client.put_object", side_effect=error):
        created, _ = personal_s3.save_structured_artifact(
            "structured-trusted",
            _structured_key(),
            {"documentId": "doc-123"},
            **_artifact_kwargs(),
...
def test_structured_artifact_proof_rejects_mismatched_metadata(
    ...
    body = personal_s3._structured_body(data)
    digest = personal_s3.hashlib.sha256(body).hexdigest()
    metadata = personal_s3._structured_metadata(
        **_artifact_kwargs(),
        content_sha256=digest,
    )
    metadata[metadata_key] = metadata_value
    with patch(
        "personal_worker.s3.s3_client.get_object",
        return_value={
            "Body": BytesIO(body),
        ...
        with pytest.raises(ValueError, match="unavailable"):
            personal_s3.require_structured_artifact_proof(
                "structured-trusted",
                _structured_key(),
                **_artifact_kwargs(),
...
def test_structured_artifact_proof_accepts_exact_metadata_and_digest():
    body = personal_s3._structured_body({"documentId": "doc-123"})
    digest = personal_s3.hashlib.sha256(body).hexdigest()
    metadata = personal_s3._structured_metadata(
        **_artifact_kwargs(),
        content_sha256=digest,
    ...
    with patch(
        ...
        assert personal_s3.require_structured_artifact_proof(
            "structured-trusted",
            _structured_key(),
            **_artifact_kwargs(),
            expected_digest=digest,
...
def test_s3_create_then_finalize_failure_is_retryable_partial_commit():
    ...
    with (
        ...
        patch("personal_worker.processors.extract.reserve_structured_artifact", side_effect=reserve),
        patch(
        ...
        patch("personal_worker.processors.extract.save_structured_artifact", side_effect=save),
        patch("personal_worker.processors.extract.update_document_status", side_effect=finalize),
        patch("personal_worker.processors.extract.sqs_client.send_message") as send,
    ):
...
def test_oversized_structured_artifact_never_finalizes_or_enqueues():
    with (
        ...
        patch("personal_worker.processors.extract.is_already_extracted", return_value=False),
        patch("personal_worker.processors.extract.reserve_structured_artifact") as reserve,
        patch(
            "personal_worker.processors.extract.get_ocr_text",
            return_value=("synthetic text", {"chars": 14, "truncated": False}),
        ),
...
def test_retry_recovers_owner_bound_partial_commit_without_bedrock():
    with (
        ...
        patch("personal_worker.processors.extract.save_structured_artifact") as save,
        patch("personal_worker.processors.extract.update_document_status") as update,
        patch(
...
def test_dynamo_update_is_conditioned_on_owner_and_domain():
    ...
    with patch("personal_worker.dynamo.dynamodb_resource.Table", return_value=table):
        reserve_structured_artifact(
            "documents", "doc-123", "personal", "structured-key", "structured",
