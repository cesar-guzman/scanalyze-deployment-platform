...
from .processors.extract import process_gov_extract_message
...
def poll_queue(queue_url: str):
    ...
    log_event("worker_started", queue="gov-extract_url")
    ...
    while not shutdown_requested:
        try:
            ...
            for msg in messages:
                ...
                try:
                    success = process_gov_extract_message(body, receipt_handle, queue_url)
                    ...
                    # process_gov_extract_message is expected to handle specific errors,
...
def main():
    ...
    logger.info("Starting gov worker")
    
    try:
        # Pre-fetch config to crash early if something is missing
        # We explicitly poll from the gov-extract_url queue for the gov tenant per design
        queue_url = config.get("queues/gov-extract_url")
