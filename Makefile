...
TOOLING_DIR  := tooling
TESTS_DIR    := tests
# Allowlist only process basics; env -i strips every inherited AWS_* binding.
GUG390_HERMETIC_ENV := env -i \
	PATH="$(PATH)" \
	HOME="$(HOME)" \
	TMPDIR="$(if $(TMPDIR),$(TMPDIR),/tmp)" \
	LANG="$(if $(LANG),$(LANG),C.UTF-8)"
...
platform-authority-upstream-prerequisites-check:
	@echo "=== GUG-376 Upstream Prerequisite Contract Check ==="
	@env -u AWS_ACCESS_KEY_ID -u AWS_SECRET_ACCESS_KEY -u AWS_SESSION_TOKEN \
		...
		-u AWS_ENDPOINT_URL_S3 -u AWS_ENDPOINT_URL_KMS \
		-u AWS_ENDPOINT_URL_SIGNER -u AWS_ENDPOINT_URL_LAMBDA \
		-u AWS_ENDPOINT_URL_SSO -u AWS_ENDPOINT_URL_IDENTITYSTORE \
		$(PYTHON) -m pytest -q $(TESTS_DIR)/test_deployment/test_gug376_*.py \
	...
	@$(PYTHON) $(TOOLING_DIR)/validate_schema.py --schemas-dir $(SCHEMAS_DIR) --fixtures-dir $(FIXTURES_DIR) --filter platform-authority-gug365-upstream-phase-authorization
	@$(PYTHON) $(TOOLING_DIR)/validate_schema.py --schemas-dir $(SCHEMAS_DIR) --fixtures-dir $(FIXTURES_DIR) --filter platform-authority-gug365-upstream-final-handoff
	@echo "GUG-376 upstream prerequisite check complete. Status: REPOSITORY_VALIDATED_NO_LIVE_EXECUTION"
...
# ── Guarded GUG-365 Live Provider Check (GUG-390, offline only) ──────
platform-authority-gug390-live-provider-check:
	@echo "=== GUG-390 Guarded Live Provider Contract Check (offline) ==="
	@$(GUG390_HERMETIC_ENV) $(PYTHON) -m pytest -q \
		$(TESTS_DIR)/test_deployment/test_gug365_phase_execution_ledger.py \
		$(TESTS_DIR)/test_deployment/test_gug390_*.py
	@$(GUG390_HERMETIC_ENV) $(PYTHON) -m py_compile \
		$(TOOLING_DIR)/platform_authority_gug365_phase_execution_ledger.py \
		$(TOOLING_DIR)/platform_authority_gug365_live_provider.py \
		$(TOOLING_DIR)/platform_authority_gug390_live_executor.py \
		scripts/deployment/platform-authority-gug390-live-provider.py
	@$(GUG390_HERMETIC_ENV) $(PYTHON) -I -S \
		scripts/deployment/platform-authority-gug390-live-provider.py --help >/dev/null
	@$(GUG390_HERMETIC_ENV) $(PYTHON) -I -S \
		scripts/deployment/platform-authority-gug390-live-provider.py inventory --help >/dev/null
	@$(GUG390_HERMETIC_ENV) $(PYTHON) -I -S \
		scripts/deployment/platform-authority-gug390-live-provider.py execute-phase --help >/dev/null
	@$(GUG390_HERMETIC_ENV) $(PYTHON) -I -S \
		scripts/deployment/platform-authority-gug390-live-provider.py reconcile --help >/dev/null
	@$(GUG390_HERMETIC_ENV) $(PYTHON) -I -S \
		scripts/deployment/platform-authority-gug390-live-provider.py certify --help >/dev/null
	@$(GUG390_HERMETIC_ENV) $(PYTHON) $(TOOLING_DIR)/validate_schema.py --schemas-dir $(SCHEMAS_DIR) --fixtures-dir $(FIXTURES_DIR) --filter platform-authority-gug365-phase-execution-ledger
...
# ── GUG-395 Pre-Plan Seed and Downstream Materializer (offline only) ──
platform-authority-gug395-preplan-seed-check:
	...
	@$(GUG390_HERMETIC_ENV) $(PYTHON) -I -S \
		scripts/deployment/platform-authority-gug395-preplan-seed.py --help >/dev/null
	@$(GUG390_HERMETIC_ENV) $(PYTHON) -I -S \
		scripts/deployment/platform-authority-gug395-preplan-seed.py catalog --help >/dev/null
	@$(GUG390_HERMETIC_ENV) $(PYTHON) -I -S \
		scripts/deployment/platform-authority-gug395-preplan-seed.py seed --help >/dev/null
	@$(GUG390_HERMETIC_ENV) $(PYTHON) -I -S \
		scripts/deployment/platform-authority-gug395-preplan-seed.py plan --help >/dev/null
	@$(GUG390_HERMETIC_ENV) $(PYTHON) -I -S \
...
platform-authority-bootstrap-plan-repair-check:
	...
	@env -u PYTHONPATH -u PYTHONHOME PYTHONDONTWRITEBYTECODE=1 $(PYTHON) -m pytest -q \
		$(TESTS_DIR)/test_deployment/test_permission_plan_migration.py \
		$(TESTS_DIR)/test_deployment/test_plan_seed_management_read_supplement.py \
	...
	@env -u PYTHONPATH -u PYTHONHOME $(PYTHON) -m py_compile \
		...
		$(TOOLING_DIR)/platform_authority_permission_plan_migration.py \
		scripts/deployment/platform-authority-permission-plan-migration.py \
...
platform-authority-bootstrap-check: platform-authority-upstream-prerequisites-check platform-authority-retirement-service-role-check platform-authority-retirement-entrypoint-check platform-authority-gug390-live-provider-check platform-authority-gug392-live-provider-check platform-authority-gug395-preplan-seed-check platform-authority-gug395-preplan-collision-check platform-authority-bootstrap-plan-repair-check
	...
	@echo "GUG-206/GUG-208/GUG-209/GUG-211/GUG-214/GUG-215/GUG-216/GUG-217/GUG-218/GUG-219/GUG-220/GUG-221/GUG-274/GUG-357/GUG-363/GUG-365/GUG-376/GUG-390/GUG-392/GUG-393/GUG-395 bootstrap check complete. Status: REPOSITORY_VALIDATED_NO_LIVE_EXECUTION"
...
docs-check: contributor-docs-check phase0-docs-check
	...
	@ERRORS=0; \
		for f in README.md REPRODUCIBILITY.md playbooks/enterprise-client-deployment.md \
			...
			docs/deployment/platform-authority-gug365-upstream-prerequisites.md \
			docs/operations/platform-authority-gug365-upstream-prerequisites.md \
			docs/security/gug376-upstream-prerequisites-threat-model.md \
